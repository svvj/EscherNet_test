...
def main(args):
    ...
    if DATA_TYPE == "Text2Img":
        ...
        obj_names = ['chair', 'drums', 'ficus', 'hotdog', 'lego', 'materials', 'mic', 'ship']
    else:
    ...
    for obj_name in tqdm(obj_names):
        ...
        if DATA_TYPE == "NeRF":
            ...
            # minimal changes: skip test_info, reuse train_info, T_out from args
            if os.path.exists(os.path.join(args.output_dir, obj_name, "output.gif")):
                continue
            with open(os.path.join(DATA_DIR, obj_name, str(T_in), "transforms_selected.json"), "r") as f:
                train_info = json.load(f)["frames"]
            with open(os.path.join(DATA_DIR, obj_name, "transforms_train.json"), "r") as f:
                test_info = json.load(f)["frames"]
            ...
            train_index = []
            # extract train index from image names
            for i in range(len(train_info)):
                idx = train_info[i]["file_path"].split("/")[-1].split("\\")[-1].split(".")[0].split("_")[-1]
                train_index.append(int(idx))
            train_index = np.array(train_index)
            # 1~100
            test_index = np.arange(0, T_out)

            # info_dir = os.path.join("metrics/NeRF_ours_idx", obj_name)
            # assert os.path.exists(info_dir), "use fixed train index"
            # train_index = np.load(os.path.join(info_dir, f"train_N{T_in}M20_random.npy"))
            # test_index = train_index  # re-use train index for output
        ...
        pose_out = []
        gt_image = []
        for tidx, T_in_index in enumerate(train_index):
            if DATA_TYPE == "RTMV":
                ...
                if DATA_TYPE == "NeRF":
                    ...
                    # change file name into 'images\\r_03d.png'-> 'images/r_03d.png'
                    train_info[tidx]["file_path"] = train_info[tidx]["file_path"].replace("\\", "/")
                    img_path = os.path.join(DATA_DIR, obj_name, str(T_in), train_info[tidx]["file_path"])
                    pose = np.array(train_info[tidx]["transform_matrix"])
                    if CaPE_TYPE == "6DoF":
                        # blender to opencv
                        pose[1:3, :] *= -1
                        pose = np.linalg.inv(pose)
        ...
        for T_out_index in test_index:
            if DATA_TYPE == "RTMV":
                ...
                if DATA_TYPE == "NeRF":
                    ...
                    OUT_DATA_DIR = "./demo/nerf_synthetic"
                    img_path = os.path.join(OUT_DATA_DIR, obj_name, test_info[T_out_index]["file_path"] + ".png")
                    pose = np.array(test_info[T_out_index]["transform_matrix"])
                    if CaPE_TYPE == "6DoF":
                        # blender to opencv
                        pose[1:3, :] *= -1
